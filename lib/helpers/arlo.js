...
const screenShotPollingIntival = 15 * 60 * 1000; // 15 minutes
...
function _setupArlo() {
  ...
  this.arlo.on(Arlo.EVENT_GOT_DEVICES, () => {
    this.logger.trace(`${this._traceStack()} - Devices ready to interact`);

    // Listen for api requests
    if (!this.apiListening) {
      this.listen();
      this.apiListening = true;

      setInterval(async () => {
        this.logger.trace(`${this._traceStack()} - Getting screen shots`);
        // Start living room stream and take snapshot
        await this.arlo.getStream(this.camLivingRoom.id);
        setTimeout(() => {
          this.arlo.getSnapshot(this.camLivingRoom.id);
        }, 5000);

        // Start garden stream and take snapshot
        await this.arlo.getStream(this.camGarden.id);
        setTimeout(() => {
          this.arlo.getSnapshot(this.camGarden.id);
        }, 5000);

        setTimeout(() => {
          this.arlo._getDevices();
        }, 10000);
