...
async function _logIn() {
  try {
    ...
    if (!sucess) {
      this.logger.error('Not able to login to Arlo');
    }
  } catch (err) {
    this.logger.error('Not able to login to Arlo');
...
async function _setupArloEvents() {
  ...
  const arloUser = 'arlo@johnpillar.me'; //await this._getVaultSecret.call(this, 'ArloUsername');
  const arloPassword = 'OVFDcTZ4NVlOOXp1'; //await this._getVaultSecret.call(this, 'ArloPassword');
  const emailUser = 'arlo'; // await this._getVaultSecret.call(this, 'emailUser');
  const emailPassword = 'Rebreather1'; // await this._getVaultSecret.call(this, 'emailPassword');
  const emailServer = '192.168.1.12'; //await this._getVaultSecret.call(this, 'emailServer');
