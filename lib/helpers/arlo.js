...
async function _processUpdate(eventData) {
  ...
  debug(`[${eventData.id}] Saving ${location} camera data`);
  const dataVaules = {
    time: new Date(),
    device: eventData.id,
    location,
  ...
  try {
    ...
    if (results.insertedCount === 1)
      this.logger.info(`[${eventData.id}] Saved data (${location})`);
    else
      this.logger.error(
        `${this._traceStack()} - [${
          eventData.id
        }] Failed to save data: ${location})`,
      );
  } catch (err) {
  ...
  this.arlo.on(Arlo.EVENT_GOT_DEVICES, (devices) => {
    ...
    for (const device of devices) {
      if (device.deviceType === Arlo.TYPE_CAMERA) {
        ...
        setTimeout(() => {
          debug(
            `[${device.deviceId}] Requesting snapshop update via starting stream`,
          );
          this.arlo.startStream(device.deviceId);
        }, 5000);
      }
    }
  });
  ...
  this.arlo.on(Arlo.EVENT_MEDIA_UPLOAD, async (media) => {
    ...
    if (presignedContentUrl) {
      ...
      try {
        ...
        await downloadFile.call(this, presignedContentUrl, folderPath);
