...
async function getImage(req, res, next) {
  ...
  try {
    ...
    if (camera.toLowerCase() === 'living room')
      imageURL = this.livingRoomCam.device.presignedLastImageUrl;

    if (imageURL === undefined) throw new Error('No image aviable');

    const image = await this._callAPIServiceGet.call(this, imageURL);
    if (image instanceof Error) {
      this.logger.error(`${this._traceStack()} - ${image.message}`);
...
async function startStream(req, res, next) {
  ...
  try {
    ...
    if (camera.toLowerCase() === 'living room') cam = this.livingRoomCam;
