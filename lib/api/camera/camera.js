 ...
 */
async function sendImageUrl(req, res, next, device) {
  setTimeout(() => {
    this.logger.trace(`${this._traceStack()} - Return new image url`);
    const imageURL = device.presignedFullFrameSnapshotUrl;
    request.get(imageURL).pipe(res);
  }, 2000);
}

function getSnapshot(req, res, next, device) {
  setTimeout(() => {
    this.logger.trace(`${this._traceStack()} - Get live snapshot`);
    this.arlo.getSnapshot(
      device,
      sendImageUrl.call(this, req, res, next, device),
    );
  }, 1000);
}
  ...
  try {
    switch (camera.toLowerCase()) {
      case 'garden':
        ...
        this._loginArlo.call(
          this,
          getSnapshot.call(this, req, res, next, this.gardenCam.device),
        );
        break;
      case 'livingroom':
        this.logger.trace(`${this._traceStack()} - Login`);
        this._loginArlo.call(
          this,
          getSnapshot.call(this, req, res, next, this.livingRoomCam.device),
        );
        break;
      case 'kids':
