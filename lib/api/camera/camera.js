...
async function getPrivacyStatus(req, res, next) {
  debug(`Get camera's privacy status API called`);

  if (!this.arlo.loggedIn) throw new Error('Not logged in');
      ...
      case 'kids':
        ...
        camImage.once('exit', async (code) => {
          ...
          try {
            ...
            if (!fs.existsSync(`${folderPath}/snapshot.jpg`))
            ...
            if (imageURL instanceof Error) {
              this._sendResponse(res, next, 500, imageURL);
              return;
            }
