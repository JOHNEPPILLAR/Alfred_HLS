  ...
  try {
    switch (camera.toLowerCase()) {
      case 'garden':
        ...
        if (imageURL instanceof Error || typeof imageURL === 'undefined') throw imageURL;
        await axios
...
function ffmpegConvert(streamURL, camera, req, res, next) {
  ...
  setTimeout(() => {
    debug(`Timout stream: ${camera} - ${uuid}`);
    stream.kill();
  }, 5 * 60 * 1000); // 5 minutes
}
...
async function startStream(req, res, next) {
  ...
  try {
    switch (camera.toLowerCase()) {
      case 'garden':
        await this.arlo.setPrivacyActive(this.camGardenID);
        setTimeout(async () => {
          url = await this.arlo.getStream(this.camGardenID);
        ...
        break;
      case 'livingroom':
        await this.arlo.setPrivacyActive(this.camLivingRoomID);
        setTimeout(async () => {
          url = await this.arlo.getStream(this.camLivingRoomID);
          if (url instanceof Error)
            throw new Error(`No stream url available: ${camera.toLowerCase()}`);
          ffmpegConvert.call(this, url, camera, req, res, next);
        }, 2000);
