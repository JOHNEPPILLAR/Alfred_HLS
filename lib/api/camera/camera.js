...
async function getImage(req, res, next) {
  ...
  try {
    switch (camera.toLowerCase()) {
      case 'garden':
        this.logger.trace(`${this._traceStack()} - Get latest image`);
        imageURL = this.gardenCam.device.presignedFullFrameSnapshotUrl;
        request.get(imageURL).pipe(res);
        break;
      case 'livingroom':
        this.logger.trace(`${this._traceStack()} - Get latest image`);
        imageURL = this.livingRoomCam.device.presignedFullFrameSnapshotUrl;
        request.get(imageURL).pipe(res);
        break;
      case 'kids':
