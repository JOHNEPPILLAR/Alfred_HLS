...
function ffmpegConvert(streamURL, camera, req, res, next) {
  ...
  mediaArgs.push(`${folderPath}/cam.m3u8`);
  ...
  debugStream('Starting stream');
  const stream = spawn('FFmpeg', mediaArgs);

  this.logger.info(`Processing stream: ${camera} - ${uuid}`);
  ...
  stream.stderr.on('data', (data) => {
    if (
      ...
      this.logger.error(mediaArgs.toString());
