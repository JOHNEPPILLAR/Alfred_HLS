...
async function getImageFromURL(req, res, next, imageURL) {
  request(imageURL, (error, response, body) => {
    if (error) {
      this.logger.error(`${this._traceStack()} - ${error}`);
      if (typeof res !== 'undefined' && res !== null) {
        this._sendResponse(res, next, 500, error);
      }
      return error;
    }
    if (response.statusCode !== 200) {
      const err = new Error(
        `Image returned status code: ${response.statusCode}`,
      );
      this.logger.error(`${this._traceStack()} - ${err.message}`);
      if (typeof res !== 'undefined' && res !== null) {
        this._sendResponse(res, next, response.statusCode, err);
      }
      return err;
    }
  ...
  try {
    switch (camera.toLowerCase()) {
      case 'garden':
        ...
        await getImageFromURL.call(this, req, res, next, imageURL);
        break;
      case 'livingroom':
        await this._loginArlo.call(this);
