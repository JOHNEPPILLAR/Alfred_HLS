  ...
  try {
    switch (camera.toLowerCase()) {
      case 'garden':
        ...
        image = await this.arlo.downloadSnapshot(this.camGarden.id);
        res.sendRaw(200, image, { 'Content-Type': 'image/jpg' });
        next();
        break;
      case 'livingroom':
        this.logger.trace(
        ...
        image = await this.arlo.downloadSnapshot(this.camLivingRoom.id);
        res.sendRaw(200, image, { 'Content-Type': 'image/jpg' });
        next();
        break;
      case 'kids':
        ...
        camImage.once('exit', async (code) => {
          ...
          try {
            image = await fs.createReadStream(`${folderPath}/snapshot.jpg`, {
              bufferSize: 64 * 1024,
            });
